...
def win_check(test_board,player,player_marker,draw):
...
def empty_position(test_board,index):
    if test_board[index]!=' ':
...
for i in range (1,10):
    if i%2!=0:
        ...
        while m==1:
            ...
            m = empty_position(test_board,index_position)
        test_board = change_board1(index_position)
        display(test_board)
        k = win_check(test_board,'Player 1',player1_marker,i)
        if k==1:
            break
        ...
        display(test_board)
        k = win_check(test_board, 'Player 2', player2_marker,i)
        if k==1:
            break
